...
OS14_HTCOM_HANDEL OS13_HTCOM::Init()
{
...
void OS13_HTCOM::Dispose(OS14_HTCOM_HANDEL h) {
    ((PIUNKNOWN)h)->Release();
    CoFreeUnusedLibraries();
    CoUninitialize();
}

ht::HtHandle* OS13_HTCOM::HT::create(OS14_HTCOM_HANDEL h, int capacity, int secSnapshotInterval, int maxKeyLength, int maxPayloadLength, const wchar_t* htUsersGroup, const wchar_t* fileName)
{
    ...
    try {
        if (!SUCCEEDED(((PIUNKNOWN)h)->QueryInterface(IID_IComHt, (void**)&pIHT)))
            ...
            if(!SUCCEEDED(pIHT->create(ht,capacity,secSnapshotInterval,maxKeyLength,maxPayloadLength, htUsersGroup,fileName)))
...
ht::HtHandle* OS13_HTCOM::HT::open(OS14_HTCOM_HANDEL h,const wchar_t* fileName, bool isMapFile)
{
    ht::HtHandle** ht = new ht::HtHandle*;
    try {
        if (!SUCCEEDED(((PIUNKNOWN)h)->QueryInterface(IID_IComHt, (void**)&pIHT)))
...
ht::HtHandle* OS13_HTCOM::HT::open(OS14_HTCOM_HANDEL h, const wchar_t* fileName, const wchar_t* htUser, const wchar_t* htPassword, bool isMapFile)
{
    ht::HtHandle** ht = new ht::HtHandle*;
    try {
        if (!SUCCEEDED(((PIUNKNOWN)h)->QueryInterface(IID_IComHt, (void**)&pIHT)))
            ...
            if (!SUCCEEDED(pIHT->openAuth(ht, fileName, htUser, htPassword, isMapFile)))
                throw std::runtime_error("Error open HT");
            else {
                pIHT->Release();
                return *ht;
...
BOOL OS13_HTCOM::HT::snap(OS14_HTCOM_HANDEL h, ht::HtHandle* htHandle)
{
    BOOL rc = false;
    try {
        if (!SUCCEEDED(((PIUNKNOWN)h)->QueryInterface(IID_IComHt, (void**)&pIHT)))
...
BOOL OS13_HTCOM::HT::close(OS14_HTCOM_HANDEL h, ht::HtHandle* htHandle)
{
    BOOL rc = false;
    try {
        if (!SUCCEEDED(((PIUNKNOWN)h)->QueryInterface(IID_IComHt, (void**)&pIHT)))
...
BOOL OS13_HTCOM::HT::insert(OS14_HTCOM_HANDEL h, ht::HtHandle* htHandle, const ht::Element* element)
{
    BOOL rc = false;
    try {
        if (!SUCCEEDED(((PIUNKNOWN)h)->QueryInterface(IID_IComHt, (void**)&pIHT)))
...
BOOL OS13_HTCOM::HT::remove(OS14_HTCOM_HANDEL h, ht::HtHandle* htHandle, const ht::Element* element)
{
    BOOL rc = false;
    try {
        if (!SUCCEEDED(((PIUNKNOWN)h)->QueryInterface(IID_IComHt, (void**)&pIHT)))
...
ht::Element* OS13_HTCOM::HT::get(OS14_HTCOM_HANDEL h, ht::HtHandle* htHandle, const ht::Element* element)
{
...
BOOL OS13_HTCOM::HT::update(OS14_HTCOM_HANDEL h, ht::HtHandle* htHandle, const ht::Element* oldElement, const void* newPayload, int newPayloadLength)
{
    BOOL rc = false;
    try {
        if (!SUCCEEDED(((PIUNKNOWN)h)->QueryInterface(IID_IComHt, (void**)&pIHT)))
...
const char* OS13_HTCOM::HT::getLastError(OS14_HTCOM_HANDEL h, ht::HtHandle* htHandle)
{
...
void OS13_HTCOM::HT::print(OS14_HTCOM_HANDEL h, const ht::Element* element)
{
    try {
        if (!SUCCEEDED(((PIUNKNOWN)h)->QueryInterface(IID_IComHt, (void**)&pIHT)))
            throw std::runtime_error("Error get interface IID_IComHt");
...
////
ht::Element* OS13_HTCOM::Element::createGetElement(OS14_HTCOM_HANDEL h, const void* key, int keyLength)
{
...
ht::Element* OS13_HTCOM::Element::createInsertElement(OS14_HTCOM_HANDEL h, const void* key, int keyLength, const void* payload, int  payloadLength)
{
    ht::Element** rcElement = new ht::Element*;
    try {
        if (!SUCCEEDED(((PIUNKNOWN)h)->QueryInterface(IID_IElement, (void**)&pIElement)))
...
ht::Element* OS13_HTCOM::Element::createUpdateElement(OS14_HTCOM_HANDEL h, const ht::Element* oldElement, const void* newPayload, int  newPayloadLength)
{
    ht::Element** rcElement = new ht::Element*;
    try {
        if (!SUCCEEDED(((PIUNKNOWN)h)->QueryInterface(IID_IElement, (void**)&pIElement)))
